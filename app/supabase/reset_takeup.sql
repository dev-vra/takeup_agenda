-- ============================================================
-- RESET COMPLETO — Projeto Supabase "takeup" (org dev-vra)
-- ⚠️  DESTRUTIVO: apaga TODOS os dados e usuários da aplicação.
-- Recria o schema do zero e cria o usuário admin inicial.
-- Cole tudo no SQL Editor e clique em Run.
-- ============================================================

-- ─── 0) LIMPEZA TOTAL ───────────────────────────────────────
-- Remove tabelas da aplicação (CASCADE derruba dependências)
DROP TABLE IF EXISTS
  report_templates, audit_log, known_responsibles, reports,
  notifications, agenda_comments, agenda_entries, documents,
  analysis_comments, takeup_reschedules, analyses,
  contract_takeup_monthly, contract_installments, contracts,
  buyers, sellers, profiles
CASCADE;

-- Remove tipos enum
DROP TYPE IF EXISTS
  audit_action, responsible_type, report_type, notification_type,
  agenda_entry_status, agenda_entry_type, installment_status,
  analysis_status, user_role
CASCADE;

-- Remove TODOS os usuários de autenticação (zera logins)
...
CREATE TYPE notification_type AS ENUM ('alerta_prazo','hvi_pendente','takeup_pendente','takeup_atrasado','parcela_vencendo','geral');
CREATE TYPE report_type AS ENUM ('contrato','analise','parcela','conjunto_analises','personalizado');
...
CREATE INDEX report_templates_user_idx ON report_templates (created_by, created_at DESC);
...
-- ─── 6) USUÁRIO ADMIN INICIAL (admin@admin.com / aQ!@#456) ───
-- O admin inicial NÃO precisa trocar a senha (must_change_password = false).
DO $$
...
BEGIN
  INSERT INTO auth.users (
    ...
    '{"name":"Administrador","role":"admin","must_change_password":false}',
    now(), now(), '', '', '', ''
  );

  -- O trigger handle_new_user já cria o profile; garante role admin e flag.
  UPDATE public.profiles
  SET role = 'admin', name = 'Administrador', must_change_password = false
  WHERE id = uid;
END $$;

-- ─── 7) VERIFICAÇÃO ─────────────────────────────────────────
SELECT p.email, p.role, p.must_change_password,

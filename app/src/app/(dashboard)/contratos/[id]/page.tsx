...
export default async function ContratoDetalhePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (process.env.NEXT_PUBLIC_USE_MOCK === 'true') {
    ...
    return <ContratoDetalhe contract={enrichedContract as any} currentUserId="mock-admin" />
  }

  const supabase = await createClient()
  const [{ data: contract }, { data: { user } }] = await Promise.all([
    supabase
      .from('contracts')
  ...
  if (!contract) notFound()
  return <ContratoDetalhe contract={contract as any} currentUserId={user?.id} />
}
